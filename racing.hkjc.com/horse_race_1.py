...
#        driver.find_element_by_id("selectId").click()
    option_menus = driver.find_element_by_id("selectId").find_elements_by_tag_name("option")
    option_id = 0
    option_len = len(option_menus)

    option_url = driver.current_url

    global writer

    while option_id < option_len:
        soup = BeautifulSoup(driver.page_source, "html.parser")
        ...
        if option_id != 0:
            ...
            ttime.sleep(2)
        option_menus = driver.find_element_by_id("selectId").find_elements_by_tag_name("option")
        option = option_menus[option_id]
        option.click()
        driver.find_element_by_id("submitBtn").click()
        ttime.sleep(2)
        option_id += 1

f = open('Scraping1.csv', 'w', newline='')
writer = csv.writer(f)
